 ...
 * Created on February 5, 2019, 9:44 PM
 * Purpose:  BlackJack Final V8
 */
...
void end(char);
float betMon(float &);
void randNum(int [],int=11); //Default Argument
...
int main(int argc, char** argv) {
    ...
    bool cont=true,goDeal=false;   //Be asked to hit again, go to dealers turn
    ifstream in;//Input File;
    ofstream out;//Output file
    
  do{                                               //Where the program repeats
      do{
          menu(start);                                   //Call Menu Function
             if(start == 's' || start == 'S'){
                  check(start);                      //Call Check Stash Function
    }
              //If the User Enters M
              if(start == 'm' || start == 'M'){         
                  add(start);                          //Call Add Money Function
    } //User Validation
      }while(start == 's' || start == 'S' || start == 'm' || start == 'M');
            //If The User Enters E
             end(start);
             if(start=='P'||start=='p') { //If user inputs (P) start game
                bet=0,sum=0,dsum=0;
                betMon(bet);
                cont=true;               //important for when user plays again
       cout<<"--------------------------------------------------------"<<endl;                              
     //Start of Game
     randNum(rand); //Calls Random Number Function| USES DEAULT ARGUMENT        
     //Value for Card 1
     card1(rand,let1);
     //Value for Card 2
     card2(rand,let2);
    ...
    if(cont){
       ...
       cout<<"Card limit reached. Input 'Y' to stand."<<endl;
        ...
        cout<<"--------------------------------------------------------"<<endl;
            cout << "Invalid Input, must enter Y ;"<<endl;
            cout << "Card limit reached. Input 'Y' to stand."<<endl;
            cin >> choice;
} 
     //if (Y) the user stays
    ...
    //Results                                                    Results
    //        
  result(sum,dsum,bet); //Result Function
...
//==============================================================================
//Generates 12 Random Numbers For Cards
//==============================================================================
...
//==============================================================================
void result(int n, int m,float b){
    ifstream in;
    ofstream out;
    float mon;
     if(n>21 && m<=21){                                       
...
char menu(char &n){
   ...
   cout<<"-To Check How Much Money You Have In Your Stash, Enter (S)"<<endl;
   cout<<"-To Exit Program, Enter (E)"<<endl;
   cout<<"NOTE: If this is your 1st time playing, please add money before"
           " starting!";
   
   cin>>n;                            //User enters (S) to start the game
   //Check to make sure user inputs (S)
    while (n != 'P' && n != 'p' && n != 'E' && n != 'e'
            && n !='m' && n != 'M' && n != 's' && n != 'S'){ 
   //If answer isn't (S,M OR E), show prompt
     cout << "Invalid Input, must be either (P), (M), (S) or (E);"<<endl;
     cout<<"Welcome to BlackJack!"<<endl;
     cout<<"-To Play, Enter (P)"<<endl;
     cout<<"-If You Would Like To Add Money To Play With, Enter (M)"<<endl;
     cout<<"-To Check How Much Money You Have In Your Stash, Enter (S)"<<endl;
